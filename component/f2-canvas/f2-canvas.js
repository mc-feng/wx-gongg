...
Component({
  ...
  methods: {
    init: function(callback) {
      const version = wx.getSystemInfoSync().SDKVersion.split('.').map(n => parseInt(n, 10));
    ...
    canvasToTempFilePath(opt) {
      this.canvas.ctx.draw(true, setTimeout(() => {
        wx.canvasToTempFilePath(Object.assign(opt, this.data), this);
      }, 100));
    },
  }
})
